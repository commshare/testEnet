...
namespace enetpp {
	...
	public:
		...
		/*有channel id ，也有客戶端的id，channel_id 就填寫當前peer發送包的sessionID，客戶端可以收到？*/
		...
		void consume_events(
			...
			if (!_event_queue.empty()) {
				...
				{
					std::lock_guard<std::mutex> lock(_event_queue_mutex);//lock TODO
					assert(_event_queue_copy.empty());
					/*在這裏消費_event_queue */
					_event_queue_copy = _event_queue;
					_event_queue = {};
				}
                //GET EVNENT FROM COPY ,NOTIFY USER
	...
	private:
       ...
       /*在線程裏，從queue裏讀取包，然後發送*/
		void send_queued_packets_in_thread() {
			if (!_packet_queue.empty()) {
				...
				while (!_packet_queue.empty()) {
					...
					/*根據包的client id 來發送，其實我們會用sessionId來高？一個sessionid會對應一個client的鏈接吧，一個client可能會創建多個session吧*/
					/*通過clientid找到所有的peer*/
					auto pi = _thread_peer_map.find(qp._client_id); //_client_id 这个是key啊，客户端的线程id么？
		...
		void capture_events_in_thread(const listen_params_type& params, ENetHost* host) {
			...
			enet_host_service(host, 0, 0); //注意，这里俩都是0！！！
		...
		void handle_receive_event_in_thread(const ENetEvent& e) {
			auto client = reinterpret_cast<ClientT*>(e.peer->data); /*私有數據做強轉爲CLIENT？ */
			if (client != nullptr) {
				std::lock_guard<std::mutex> lock(_event_queue_mutex);
