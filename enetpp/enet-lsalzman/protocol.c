...
{
    while (! enet_list_empty (& host -> dispatchQueue)) //如果dispatchQueue不空
    {
...
应该定期调用enet_host_service以获得足够的性能
    @ingroup host
*/
int
enet_host_service (ENetHost * host, ENetEvent * event, enet_uint32 timeout)
{
    enet_uint32 waitCondition; //还没填
    ...
    host -> serviceTime = enet_time_get (); //当前时间
    
    timeout += host -> serviceTime;  //绝对时间？

    do
    {
       if (ENET_TIME_DIFFERENCE (host -> serviceTime, host -> bandwidthThrottleEpoch /*
带宽节流时代*/) >= ENET_HOST_BANDWIDTH_THROTTLE_INTERVAL)
       ...
       if (ENET_TIME_GREATER_EQUAL (host -> serviceTime, timeout))//大于等于，超时了，退出函数
         return 0;
       ...
       do
       {
          ...
          if (ENET_TIME_GREATER_EQUAL (host -> serviceTime, timeout)) //超时了，退出函数
            return 0;

          waitCondition = ENET_SOCKET_WAIT_RECEIVE | ENET_SOCKET_WAIT_INTERRUPT;
          // select 操作，阻塞的，没有数据会return -1 退出，会根据收到的事件填充waitCondition
          if (enet_socket_wait (host -> socket, & waitCondition, ENET_TIME_DIFFERENCE (timeout, host -> serviceTime)) != 0)
            return -1;
       }
       while (waitCondition & ENET_SOCKET_WAIT_INTERRUPT); //没有收到interupt事件，会结束这个while

       host -> serviceTime = enet_time_get ();
    } while (waitCondition & ENET_SOCKET_WAIT_RECEIVE);//如果一直有数据发来，不会退出这个while
